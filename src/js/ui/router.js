...
const Router = {
    ...
    async linkClick(el, e) {
        ...
        let legacyLinks = []
        const legacyLinksEl = document.querySelector('meta[name="legacy_links"]')
        if (legacyLinksEl) {
            legacyLinks = legacyLinks.concat(JSON.parse(document.querySelector('meta[name="legacy_links"]').content))
        }

        let legacy = false
        legacyLinks.forEach((link) => {
            if (urlObj.pathname.startsWith(link)) {
                legacy = true
            }
        })
